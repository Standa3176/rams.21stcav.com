...
class MethodStatementService
{
    ...
    private function buildScope(array $parsed, array $classified): string
    {
        ...
        $primaryScope = trim((string) ($parsed['scope_of_works'] ?? ''));

        if ($primaryScope === '' && ! empty($parsed['tasks']) && $pmNotes === '') {
            // tasks[] without pm notes means the parser populated them from equipment, not user input
            $primaryScope = implode('; ', array_slice($parsed['tasks'], 0, 5));
        }

        if ($primaryScope === '' && ! empty($classified['summary'])) {
            $primaryScope = $classified['summary'];
        }
            ...
            'phases' => [
                ...
                [
                    ...
                    'steps' => [
                        ...
                        'Survey wall substrates, select fixings appropriate to the surface type, and mount displays and screens using the team size required for the panel ('
                            . DisplayLiftPolicy::compactBandSummary() . '), torquing fixings to manufacturer guidance.',
