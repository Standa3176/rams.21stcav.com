...
class OmManualDocxService
{
    ...
    private function buildScopeTable(Section $s, array $rooms): void
    {
        ...
        // Header
        $header = $table->addRow();
        foreach (['Room / Area', 'Dwg Ref', 'Installed System'] as $i => $heading) {
            $widths = [2200, 1200, 5100];
            $cell = $header->addCell($widths[$i], ['bgColor' => self::TEAL]);
            $cell->addText($heading, ['name' => 'Arial', 'size' => 9, 'bold' => true, 'color' => self::WHITE]);
        }

        foreach ($rooms as $room) {
            $equipmentStr = implode('; ', array_map(
                fn (array $eq) => ($eq['qty'] > 1 ? $eq['qty'] . '× ' : '1× ') . $eq['model'],
                $room['equipment'] ?? []
            ));

            $row = $table->addRow();
            $row->addCell(2200)->addText($room['name'] ?? '', ['name' => 'Arial', 'size' => 9]);
            $row->addCell(1200)->addText($room['drawing_ref'] ?? '—', ['name' => 'Arial', 'size' => 9]);
            $row->addCell(5100)->addText($equipmentStr, ['name' => 'Arial', 'size' => 9]);
        }
    }
    ...
    private function buildAssetRegisterSection(Section $s, array $rooms, int $num): void
    {
        ...
        $header = $table->addRow();
        foreach (['Room', 'Qty', 'Equipment', 'Model / Part No.', 'Serial No. (to complete)'] as $i => $heading) {
            $widths = [2000, 500, 2000, 2800, 1700];
            $cell = $header->addCell($widths[$i], ['bgColor' => self::TEAL]);
            $cell->addText($heading, ['name' => 'Arial', 'size' => 9, 'bold' => true, 'color' => self::WHITE]);
        }
        ...
        foreach ($rooms as $room) {
            foreach ($room['equipment'] ?? [] as $eq) {
                ...
                $row->addCell(2000)->addText($room['name']      ?? '', $small);
                $row->addCell(500) ->addText((string) ($eq['qty'] ?? 1) . '×', $small);
                $row->addCell(2000)->addText($eq['description'] ?? '', $small);
                $model = trim(($eq['model'] ?? '') . ($eq['part_no'] ? ' ' . $eq['part_no'] : ''));
                $row->addCell(2800)->addText($model, $small);
                $row->addCell(1700)->addText('', $grey);
            }
        }
    }
