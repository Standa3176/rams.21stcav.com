...
- [ ] **GATE-13**: Hot-works contradiction — a RAMS asserting "No hot works of any kind included in this scope" while also requiring a hot-works permit, or listing solder/flux in COSHH, is self-contradictory and must error. Same contradiction class as the shipped GATE-08. *(Found 2026-08-25 in 21CQ30960: RA18 says no hot works, §6.8 requires a hot-works permit for soldering, and COSHH carries Tin/Lead solder and rosin flux — three sections disagreeing.)* *(Phase 30 in progress, 2026-09-13: scheduled for Plan 30-07, to be built whole with both halves and full test coverage behind its own `RAMS_HOT_WORKS_GATE` flag. Per 30-CONTEXT.md D-02 that flag will default `false` and is NOT flipped until Phase 31, once RULE-05/GATE-10 make the COSHH table job-conditional. **Recording this now so a future reader does not read "code-complete" as "shipped and armed" — same status ambiguity as GATE-11/GATE-12 at `:72`.**)*
- [ ] **GATE-14**: Missing risk references — every method step must cite the hazards its own text implies. GATE-03 (shipped) checks that cited references *resolve*; nothing checks that a step is *missing* a reference it plainly needs. *(Found 2026-08-25 in 21CQ30960: Step 4 "Display & Mount Installation" cites RA11/12/13/21 but omits RA01 Working at Height and RA02 Manual Handling, on a step entirely about lifting displays onto wall mounts.)* *(Phase 30 in progress, 2026-09-13: scheduled for Plan 30-08, to be implemented as a WARN-tier gate (not error — `associated_risks` is app-generated, so a violation is unactionable from the review screen) behind its own `RAMS_MISSING_RISK_REF_GATE` flag, which will default `false` per D-03 pending live corpus measurement (`30-MEASUREMENT.md`). **Built-but-disarmed once shipped — same status ambiguity as GATE-11/GATE-12 at `:72`.**)*
...
| GATE-13 | Phase 30 | Pending (added 2026-08-25; in progress — scheduled Plan 30-07; ships built-but-disarmed per D-02, armed in Phase 31) |
| GATE-14 | Phase 30 | Pending (added 2026-08-25; in progress — scheduled Plan 30-08; ships as WARN-tier, disarmed per D-03) |
