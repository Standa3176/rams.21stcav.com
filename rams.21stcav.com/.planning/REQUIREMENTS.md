...
| DC-06 | Phase 46.2 | **Partially verified - WIRING COMPLETE, THREE OF FOUR ARTIFACTS UNOPENED.** (Plans 46.2-02 + 46.2-05; 46.2-06 verified as far as it was authorised to, 2026-09-24.) Seven of eight Word x PDF cells are wired to generators that already existed - no new generator, no second renderer - and `DocumentFormatInventoryTest` asserts `Route::has()` on every non-null cell with the missing set asserted to be exactly `['worksheet.pdf']`. `site-surveys.docx` is newly reachable (the service had ZERO callers). **What is NOT verified: only the WORKSHEET `.docx` has been produced and OPENED** (46.2-06 builds it through `WorksheetDocxService`, downloads it through `worksheets.download`, opens it as a zip, and asserts `word/document.xml` plus the project's own data inside it). The RAMS, O&M and site-survey artifacts were **NOT opened**, because `BuildRamsDocumentJob`, `BuildOmManualJob` and `BuildWorksheetJob` step 1 call the model and that spend was not authorised for this plan. Deliberately NOT flipped to Complete: the requirement says the documents generate, and three of them are proven only up to the generator boundary. **NOTE for whoever lifts this:** `site-surveys.docx` -> `SiteSurveyDocxService` contains no model call and is ALREADY exercised for real by `DocumentFormatInventoryTest::test_site_survey_word_downloads_a_docx` (200, `.docx` attachment) - so the site-survey Word cell can be closed for FREE by adding the zip-open assertion; it never needed AI spend |
...
| DC-11 | Phase 46.2 | Complete WITH ONE NAMED EXCEPTION (Plans 46.2-01 / -03 / -05, reconciled by 46.2-06, 2026-09-24). Every moved count is in `46.2-COUNT-LEDGER.md` with its old value, new value, plan and reason - including the ones that did NOT move and were re-taken. No `assertGreaterThanOrEqual` was introduced anywhere and the one that existed (F-7) became an exact positive. **The exception, found by reconciling against the code rather than against the summaries:** `DEFERRED_AFFORDANCES` is 21 entries but has **no `assertCount` anywhere in the suite**, unlike the other three fence lists - so an entry could LEAVE the list with every test still green. Three phases have described it as an exact count; it has never been one. Raised as **D-46.2-06-04** with the line number and the one-line fix, not fixed in a phase-closing plan |
