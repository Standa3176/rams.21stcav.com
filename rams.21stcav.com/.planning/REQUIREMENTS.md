...
- [ ] **RULE-09**: Electrical scope boundary stated — works terminate at existing socket or client data outlet, no alteration to fixed installation, no live working. **PARTIAL — 2026-09-06, Plan 28-05:** the sentence itself is now unconditionally seeded as a 6th bullet in `RamsDisplayPatchService`'s default `exclusions` array, proven to reach the live DOCX render path (`buildLegacy()`) for a never-reviewed document. **Deliberately stays `[ ]` Pending** — the `!isset` seed only fires once, before a document's first Save Review; every document that has already been reviewed even once has `reviewed_data['exclusions']` permanently `isset`, so the new bullet never reaches it. Plan 28-07's backfill migration is the remaining work. D-07 also explicitly deferred four further `house-rules.md` positions (BS 7671/lock-off wording — open user decision pending; live-working PPE-row ban; "first-fix power"→"first-fix AV signal/data/ELV cabling" rename; hardwired-supply client-isolation wording) — none built, all named rather than silently dropped. See `28-05-SUMMARY.md`.
...
| RULE-09 | Phase 28 | Pending — PARTIAL (Plan 28-05, 2026-09-06: boundary sentence shipped, unconditional for never-reviewed documents; already-reviewed-document gap remains, Plan 28-07 owns backfill) |
