...
- [ ] **GATE-09**: Display lift that does not conform to RULE-02's bands → error. Specifically: 1 operative for a display at any size → error; 4 or more operatives at any size → error; 2 operatives for a display above 90″ → error. An unresolvable display size is **not** an error (it takes the 2-operative floor silently — see 27-CONTEXT.md D-05). *(Amended 2026-08-25 alongside RULE-02; originally read "anything other than two-operative → error".)*
