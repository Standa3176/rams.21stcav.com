...
**Group DL closed by Plan 46.3-04 on 2026-09-26.** All six are Complete in code and in test. Two
things are deliberately NOT closed with them, and both are recorded rather than absorbed:

- **ROADMAP criterion 3 as literally written — "no module row shows a visit count or visit phrase" —
  was NOT DELIVERED, because it was SUPERSEDED.** It predates the revision of D-04. The user chose
  hide-at-zero over a full strip on 2026-09-26, after planning found that a strip would have deleted
  the page's only at-rest disclosure of a reconstructed or superseded visit (24 backfilled visits on
  live). **DL-04 is the authority on what shipped, and DL-04 is delivered in both directions.** The
  ROADMAP line is left as written and marked superseded in `46.3-COUNT-LEDGER.md` rather than quietly
  edited to match the outcome.
- **`0 documents` still renders on the RAMS and O&M rows, and that is an OPEN QUESTION, not a gap.**
  The zero rule is scoped to the visit phrase because the user's ruling named the visit phrase, and a
  planner does not widen a user's decision on their behalf. `CockpitSpineTest` asserts `0 documents`
  on both rows deliberately, so the day it changes is a day somebody edits a test on purpose. Carried
  verbatim to Plan 46.3-04's checkpoint as question 2.

**Three things this phase did not touch and does not claim:** DC-07 (no worksheet PDF exists, still
unchosen), the Worksheet retitle (46.2 ruling (b), still unanswered), and `D-46.2-06-01` (the
...
| VL-03 | Phase 46 | Complete (Plans 46-04 + 46-05 + 46-08, 2026-09-21). 46-08 closed it: the end-to-end walk opens `/survey/{token}` and `/worksheet/{token}` UNAUTHENTICATED at five points across the lifecycle and closes on a byte-identical comparison of both `access_token`s through `getRawOriginal()` — no office act rotated, reissued or invalidated a live link. 46-05 proved both links still return 200 for the pre-existing cases (no visit, no survey, legacy worksheet). 46-04: no token became mass-assignable (S-02/S-03 `$fillable` omissions untouched, tokens still minted only in `boot::creating`), and a test asserts no token is rendered anywhere in the cockpit region |
| VL-04 | Phase 46 | Complete (Plans 46-03 + 46-08, 2026-09-21) — **the phase's core feature**, D-01. `SurveyCarryForward` puts all ten survey fields on the install engineer's unauthenticated `/worksheet/{token}` link, READ LIVE from the `SiteSurvey` record and never copied: 46-03 proved it against a fixture, and 46-08's end-to-end walk proves it against a survey the engineer actually submitted through the public endpoint — edit the survey mid-walk and the link shows the new wording with the superseded value gone. `office_review_notes` is excluded BY NAME, because that page is signed by the client |
| VL-05 | Phase 46 | Complete (Plans 46-01 + 46-06, 2026-09-20). Accept records `accepted_at` + `accepted_by_user_id`, logs one `visit_accepted` row, closes the visit for the progress ring, and leaves the stored `status` vocabulary and every engineer record byte-identical. The lock is the sentence "Scope locked - returned {date}", never a disabled control |
| VL-06 | Phase 46 | Complete (Plans 46-01 + 46-05 + 46-06, 2026-09-20). Send back stores `sent_back_at` + one `send_back_reason` (required, 5..2000) and NOTHING else; `submitted_at` is never cleared, so the reopening stays a comparison and a resubmission relocks with no flag to clear. 46-05's engineer banner is reachable in the product for the first time |
| VL-07 | Phase 46 | Complete (Plan 46-07, 2026-09-20). An office note goes to the append-only `visit_notes` table — `created_at` only, `UPDATED_AT = null`, authored, no update path and no delete path anywhere in `app/` or the route table. `site_surveys.office_review_notes` was deliberately NOT reused: single-valued and overwritable, author-less, on the engineer's record, and with no worksheet equivalent. D-02 is executable — `submitted_at`, `survey_data`, `office_review_notes`, `access_token` and `updated_at` are compared through `getRawOriginal()` across a note write, at the model AND the HTTP boundary |
| VL-08 | Phase 46 | Complete (Plans 46-02 + 46-07, 2026-09-20). `Raise a snag` creates ONE `open` snag with three fields, linked to the visit it came from, plus one `snag_raised` activity row. D-03 is fenced twice: 46-02's nine-column schema fence, and 46-07's HTTP boundary — `outcome`, `parts`, `parent_snag_id`, `assigned_to`, `cost`, `resolved_at` and a posted `status` are all IGNORED, asserted per field. No outcome, no parts relation, no follow-up chain and no register: Phase 47 extends this record |
...
| RV-01 | Phase 46.1 | Complete (Plans 46.1-01 + 46.1-03, closed by 46.1-06, 2026-09-22). The `Returned` tab renders the visit's photos as one contact sheet per bucket, its per-room answers and notes, its captured serials and the client's name and signature. 46.1-06's end-to-end walk is the proof that it is the ENGINEER'S bytes on the page: two room photos, a confirmed serial and a signature all arrive through the real UNAUTHENTICATED `/worksheet/{token}` endpoints, and the tab then shows every one of them. The tab appears IF AND ONLY IF the open module's drawer holds a visit with a `source_type`, so no module ever carries a permanently empty fourth tab |
| RV-02 | Phase 46.1 | Complete (Plans 46.1-01 + 46.1-03, closed by 46.1-06, 2026-09-22) — D-04, derived over stored. `VisitEvidence` reads the survey / worksheet / label rows at call time and copies nothing onto `visits`: edit the engineer's record and the tab changes with it. 46.1-06 closes the other half, which is the one that matters (D-02): `worksheets.access_token`, `worksheet_signoffs.signature_png_base64` and EVERY `worksheet_photos.filename` are byte-identical through `getRawOriginal()` across a render, a ZIP download, three photo GETs and a real accept — and `/worksheet/{token}` is still 200 unauthenticated afterwards |
| RV-03 | Phase 46.1 | Complete (Plans 46.1-01 + 46.1-03, 2026-09-22). Serials are a LIST — room · device · serial · when — not thumbnails, because they are the thing most likely wanted in Bitrix and the office can see them nowhere else. `device_label_photos.captured_by` is not merely unrendered: it is not a KEY in the evidence payload, and a serialised-output assertion fails if one is ever added anywhere in the tree. A serial not yet read says so rather than rendering an empty cell |
| RV-04 | Phase 46.1 | Complete (Plans 46.1-02 + 46.1-04, corrected and closed by 46.1-06, 2026-09-22). One authenticated GET streams every photo on the visit as `{Room}/{before|after|label}/{nnn}-{name}`, with a README carrying the included and skipped counts; `Download` was lifted from the fence by name and by exactly one entry. 46.1-06's walk opened the produced archive for real and found the hand-off was INCOMPLETE on live: label photos are written to the `public` disk while the resolver looked on `local` (siblings in Laravel 11+), so every equipment-label photo was silently skipped from the ZIP and 404ed on the inline photo route. `VisitEvidence::DISK_FOR_KIND` now carries a disk beside every path and `resolveWithinDisk()` checks containment against that disk's root |
| RV-05 | Phase 46.1 | Complete (Plan 46.1-05, closed by 46.1-06, 2026-09-22) — D-02. Accept · Send back · Add note · Raise a snag render ONLY at the foot of the Returned tab, under the photos, the answers, the serials and the signature, so pressing Accept means having scrolled past what came back from site. `visit-row`'s `controls` prop defaults to FALSE and `returned-tab` is its only caller; Overview keeps every state sentence and offers nothing. 46.1-06 fixed what made the relocation unusable: every act redirected to the module's TABLESS URL, which resolves to Overview, so using a control threw the PM off the tab it lived on. The tab now rides in a hidden field, membership-checked against `ProjectCockpitController::TABS` on the way in and on the way out |
...
| DC-01 | Phase 46.2 | Complete (Plan 46.2-01, proven through HTTP by 46.2-06, 2026-09-24). `MODULE_MAP` reduced nine -> four, reversing D-16 and narrowing D-11. 46.2-06 asserts it over the RENDERED page - exactly four `cav-module__title`, `assertCount(4, moduleMap())` beside it, and each of the five removed rows asserted ABSENT rather than merely unlisted. The denominator stays derived (`CockpitSpineTest:658` uses `count($modules)`), so nine -> four needed no edit there and could not have been hidden there either |
| DC-02 | Phase 46.2 | Complete (Plan 46.2-01, 2026-09-22). Phase 45's every-`Visit::TYPE_*`-reaches-exactly-one-row invariant retired BY NAME citing 46.2 D-01, never deleted. Both surviving halves re-asserted: no type reaches two rows, and the zero-set is the named allow-list `[TYPE_COMMISSIONING, TYPE_PROGRAMMING, TYPE_SNAG]` (`CockpitModulePresenterTest:211`), so a seventh type added later still fails loudly |
| DC-03 | Phase 46.2 | Complete (Plan 46.2-04, 2026-09-23). `CockpitDocumentFormPresenter::DOCUMENT_FIELD_MAP` - one row per document type. 46.2-05's Blade renders it and contains NO document literal: it switches on `$field['type']` against the map's seven `TYPE_*` constants, asserted by grep with Blade comments stripped first. The next change is a map row, not a rewrite |
| DC-04 | Phase 46.2 | Complete (Plan 46.2-04, 2026-09-23) - every mapped field names a real consumer in a real generator, proven by a test that greps the named generator for the named key. The user's own examples (install dates, engineers, site contact) are RAMS fields and were not duplicated onto the Worksheet; the panel says so instead of leaving a short form looking broken |
| DC-05 | Phase 46.2 | Complete (Plan 46.2-05, proven through HTTP by 46.2-06, 2026-09-24). `?action=generate` discloses the form; `POST projects/{project}/cockpit/documents` validates from the map, persists where the generator reads, and DELEGATES. No JavaScript, all nine handler attributes still banned, `@csrf` on every form (fence asserts an exact 4), every lookup scoped to the route-bound `{project}`. 46.2-06 walks GET -> disclosure -> POST -> redirect for all four documents |
| DC-06 | Phase 46.2 | **Partially verified - TWO OF FOUR DOCUMENTS PROVEN TO OPEN.** (Plans 46.2-02 + 46.2-05; 46.2-06 verified as far as it was authorised to, 2026-09-24.) Seven of eight Word x PDF cells are wired to generators that already existed - no new generator, no second renderer - and `DocumentFormatInventoryTest` asserts `Route::has()` on every non-null cell with the missing set asserted to be exactly `['worksheet.pdf']`. **OPENED FOR REAL, to the same standard (non-zero bytes, `ZipArchive::open()` true, `word/document.xml` present, the project's and the PM's own typed data inside the XML): the WORKSHEET `.docx` and the SITE SURVEY `.docx`.** Both writers are pure PhpWord with no model call. **NOT OPENED, and NOT VERIFIED: the RAMS (Word and PDF), the O&M manual (Word and PDF), and the site-survey PDF.** The three AI documents build their content through `BuildRamsDocumentJob` / `BuildOmManualJob` / `BuildWorksheetJob` step 1, which call the model, and that spend was not authorised; the site-survey PDF is Browsershot and was not authorised either. Deliberately NOT flipped to Complete: the requirement says the documents generate in Word AND PDF, and four of the eight cells have never been opened by anyone |
| DC-07 | Phase 46.2 | **GAP - NOT DELIVERED, and still unchosen.** No worksheet-document PDF path exists: `worksheets.engineer-report-pdf` renders the engineer's *activity* report and `abort_if(! hasEngineerActivity(), 404)` fires for exactly the PM this phase serves, and there is no `pdf/worksheet*.blade.php`. 46.2-05 made the panel SAY so - `PDF is not available for this document (DC-07).` - and refuse `format=pdf` server-side as well, so a hand-crafted POST cannot ask for a document that cannot be rendered. The three options are costed in `46.2-FORMAT-INVENTORY.md`; **none is chosen, and 46.2-06 did not choose one.** Needs the user's call |
| DC-08 | Phase 46.2 | Code-complete; **AWAITING THE HUMAN CHECK** (Plan 46.2-05 builds it, checkpoint presented by Plan 46.2-06, 2026-09-24 - NOT self-approved, exactly as VL-11 and RV-08 were not, because "simple to use" is the user's own wording and an earlier design was rejected as "scary"). What was built to keep it calm: fields GROUPED (5 fieldsets on the widest document), display-only values rendered as TEXT rather than inputs so the PM is never asked what the project already answers, engineer/programmer pickers rendered from live `LabourResource` names (LR-04: names only, never an email or a phone), the Worksheet row carrying ZERO inputs and one sentence explaining why, and the missing format STATED rather than offered. Unsettled by any assertion and therefore unsettled |
...
| DL-01 | Phase 46.3 | Complete (Plan 46.3-02, walked through HTTP by 46.3-04, 2026-09-26). The panel is rendered INSIDE the module loop, immediately after its own row, and `.cav-layout` stopped being the panel's column. Proven by POSITION, not by count: `CockpitPageTest::test_the_open_panel_renders_inside_the_module_list_immediately_after_its_own_row()` and, through HTTP, `CockpitInlineDrawerEndToEndTest::test_opening_a_module_renders_its_drawer_beneath_that_row_and_collapses_the_others()`, which walks the DOM children of `.cav-modules` and asserts the element following the row IS the drawer. That assertion was PROVEN TO BITE by inserting a probe element between the row and the panel (2 failed), then restoring the Blade from a file copy, md5 identical. Still query-string state; still no JavaScript |
| DL-02 | Phase 46.3 | Complete in code and in test; **the word "obvious" is the user's to judge** (Plan 46.3-02, walked by 46.3-04, 2026-09-26). The loop skips every non-open row, so the others are NOT RENDERED rather than hidden — `CockpitReadOnlyFenceTest::test_the_region_holds_every_module_row_closed_and_exactly_one_open()` pins `count(moduleMap())` closed and exactly 1 open, derived and never a literal, and it was proven to bite by removing the loop skip. The way back is a NAMED anchor with visible text (`Back to all modules`) above the tab strip, plus the header `×` — two ways out — asserted on every open region by `CockpitPanelTest::test_the_way_back_and_the_close_are_on_every_open_region()`, which also asserts the CLOSED page carries neither. **46.3-04 walks the ROUND TRIP**: `CockpitInlineDrawerEndToEndTest::test_the_way_back_anchor_returns_to_the_bare_page_with_every_row()` parses the anchor's OWN href out of the response and issues it, so the CONTROL is proven and not just the route. Whether a PM FINDS it without hunting is checkpoint step 5 and is not self-answered |
| DL-03 | Phase 46.3 | Complete (Plan 46.3-03, walked through HTTP by 46.3-04, 2026-09-26). `x-cockpit.activity-panel` is a sibling of `.cav-modules` inside `.cav-layout`, rendering unconditionally with or without `?module=`; the module panel carries no feed at all. Three assertions replaced the one retired (`CockpitPanelTest:287`): exactly one panel on the bare page, exactly one on every module x tab, and NONE inside the module panel — the third asserts the panel IS there first, so it cannot pass vacuously. `CockpitPanelPresenter::activity()` was not renamed, re-signatured or re-derived, and `activity-row.blade.php` is unchanged to the byte. **46.3-04 asserts SUBTREE EQUALITY of the panel across all six steps of the walk**, because a different feed of the same length satisfies a count |
