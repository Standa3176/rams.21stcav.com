...
- **RESOLUTION (2026-07-08):**
  1. `MethodStatementPrompt::build()` now wraps every user-controllable field (`site_address`, `scope`, `equipment_summary`, `hazard_summary`, `rooms`, `room_overview_summaries`, `works_overview`, `room_descriptions`, and every scope-bucket item name) in `<<user_data>>…<<end_user_data>>` sentinel tags. The system message tells the model that any content between the tags is untrusted data and never carries instructions. `activities` stays untagged because it's a classifier enum, not user text. Adversarial content that contains a sentinel is neutralised (`<<end_user_data>>` → `<<end_user_data_>>`) before interpolation so it can't close the block early.
  2. `MethodStatementGeneratorService::isValid()` now enforces two new guards. Guard 6 rejects any response that omits any of the mandatory Step-1 H&S phrases (`toolbox talk`, `asbestos`, `permit-to-work`, `assembly point`, `ppe`) — this is the direct H&S-liability hedge the audit called out. Guard 7 rejects any response containing injection-attempt markers (`ignore the above`, `disregard previous`, echoed `<<user_data>>` sentinels, etc.). Both guards use case-insensitive substring matching; a failure triggers retry, and if the retry also fails the static 6-phase fallback is returned. `failureReason()` mirrors the guards for observability.
  3. `tests/Unit/Services/MethodStatementInjectionDefenceTest` (13 cases, 24 assertions) exercises both layers: prompt-side wrapping + neutralisation + system-message brief, plus every mandatory-phrase omission and every injection-marker echo. All pass.

  Deferred: the "AI-mentioned equipment must appear in the reviewed equipment list" cross-check. Generic terms ("displays", "speakers", "cabling") and abbreviated quote-derived names give too many false positives without a proper name-matching layer — worth a follow-up plan of its own.
