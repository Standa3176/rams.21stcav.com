...
**Requirements**: DRAW-50, DRAW-51, DRAW-52, DRAW-53
**Success Criteria** (what must be TRUE):
  1. Importing a quote whose `line_items` contains a part_number NOT in `device_stencils` creates a new stub row in `device_stencils` + N rows in `device_ports` derived from a category template, idempotent across re-imports (verified by feature test against a fresh DB + the Light Forms 21CQ30451-01-OPS fixture)
  2. The category template chooser uses ONLY deterministic signals (part_number prefix, description keywords from a fixed allowlist) — never AI invention — so the same import always produces the same stub shape; ambiguous categories produce a zero-port stub rather than a wrong guess
  3. Admin can browse `/admin/stencils?source=auto-generated&needs_review=1`, see a list of every stub awaiting promotion, click into one, edit its ports (drag to reposition, edit label/signal_type/connector_type/direction inline, add/delete rows), upload a manufacturer logo SVG, and click "Promote to engineer-curated" — `device_stencils.source` flips, `needs_review` clears, audit row written
  ...
  5. The 10 highest quote-volume part_numbers from existing imports (computed via `php artisan stencils:coverage-report`) have Tier 1 (full port) coverage at phase close — engineer-driven fill via Plan 24-02's UI, bounded delivery target rather than a 91-device sprint
  6. No regression on Phase 23's renderer: projects whose devices remain Tier 2 (auto-generic) continue to render with the placeholder; D-07 NULL-FK cable fallback unchanged
**Plans**: 3 plans, 2 waves (provisional — firm during /gsd-discuss-phase 24)
  - [ ] 24-01-PLAN.md — Quote-import auto-stub flow: ExtractQuoteJob hook calling DeviceStencilCacheService::firstOrCreate per line_item part_number; CategoryPortTemplateResolver service (deterministic, allowlist-driven, zero-AI); needs_review boolean column on device_stencils + migration; coverage-report artisan command (`stencils:coverage-report` — outputs top-N part_numbers by quote occurrence + Tier 1/2 split). Wave 1 — foundation. Requirements: DRAW-50, DRAW-51.
  - [ ] 24-02-PLAN.md — Admin curation UI: `/admin/stencils` list route with filter chips (source / needs_review / manufacturer); edit screen using Alpine.js drag-port handles + inline field edits + logo-SVG file upload + signal_type/connector_type/direction dropdowns from config; "Promote to engineer-curated" + "Discard & regenerate" actions; audit log row per edit. Wave 2 — depends on 24-01. Requirements: DRAW-52, DRAW-53.
  - [ ] 24-03-PLAN.md — Tier 1 fill bounded delivery: engineer-driven curation of the top-10 highest-volume part_numbers identified by `stencils:coverage-report`, using the Plan 24-02 UI; per-stencil verification that promoted port shape renders correctly in Phase 23 drawings (visual diff against a known-good baseline drawing for each device). Wave 2 — parallel with 24-02 closeout once UI is functional. Requirements: none (bounded delivery task — see 24-CONTEXT.md D-13; DRAW-54 is Phase 25 scope).
**UI hint**: yes (admin route + edit screen + drag-port handle UX; quote-import flow is backend but surfaces a "stubs created" toast to the importer)
**Canonical refs**:
  ...
  - `app/Jobs/ExtractQuoteJob.php` (auto-stub hook insertion point)
