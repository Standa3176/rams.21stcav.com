...
<div class="page-header">
    ...
    <div style="display:flex;gap:.5rem;align-items:center;">
        ...
        <a href="{{ route('rams.index') }}" class="btn btn-outline btn-sm">← Back to RAMS</a>
    </div>
</div>
...
<form id="review-form" method="POST" action="{{ route('rams.approve', $rams) }}" novalidate>
    ...
    <div class="review-section">
        <div class="review-section-header">
            <h2>2. Equipment</h2>
            <button type="button" class="btn btn-outline btn-sm" onclick="addRow('equipment-tbody', equipmentRowTemplate)">
                + Add Row
            </button>
        </div>
        <div class="review-section-body" style="padding:0;overflow:hidden;">
            ...
            <table class="repeater-table">
                ...
                <tbody id="equipment-tbody">
                    @foreach ($reviewPayload['equipment'] as $i => $item)
                        <tr>
                            <td class="col-qty">
                                <input type="number"
                                       name="equipment[{{ $i }}][quantity]"
                                       value="{{ old("equipment.{$i}.quantity", $item['quantity']) }}"
                                       min="1" max="999">
                            </td>
                            <td>
                                <input type="text"
    ...
    <div class="action-bar">
        ...
        {{-- Approve & Generate: posts to the approve endpoint (default form action) --}}
        <button type="submit"
                id="btn-approve"
                class="btn btn-teal"
...
// ─── Row templates ────────────────────────────────────────────────────────────
function equipmentRowTemplate(idx) {
    return `<tr>
        <td class="col-qty">
...
function confirmApprove() {
    return confirm(
        'Approve this data and generate the RAMS document?\n\n' +
        'The AI will generate the method statement based on the activities and hazards you have reviewed. ' +
        'This will queue the document for generation — you will not be able to edit the reviewed data after approval.'
    );
}
</script>
@endpush
