<!DOCTYPE html>
{{--
    Phase 260726-rf3 Plan 03 — Unified RAMS PDF blade (v2).

    This blade is the RAMS_UNIFIED_COMPOSER=on render path. It receives:

    - $dto   (App\Support\Rams\RamsDocumentDTO)  — canonical typed data
    - $theme (App\Support\Rams\RamsTheme)        — design tokens
    - $rams  (App\Models\RamsDocument)           — legacy access for the
                                                   compliance-upgrade fields
                                                   still to be ported to the
                                                   DTO in Plan 05.
    ...
    document while both are in play. Snapshot tests
    (tests/Snapshot/Rams/PdfSnapshotTest.php) guard this invariant.
--}}
<html lang="en">
<head>
<meta charset="UTF-8">
<title>RAMS &mdash; {{ $rams->project_name }}</title>
{{-- Theme tokens exposed as CSS custom properties — one source of truth for
     every hex colour, font family, and point size the v2 blade consumes.
     Emitted BEFORE the local <style> block so the local rules can override
     via cascade order when needed. --}}
{!! $theme->paletteCss() !!}
<style>
/* ── Base ─────────────────────────────────────────────────────────────────── */
...
@php
    ...
    // are populated by RamsComplianceUpgradeService::upgrade() and are not
    // yet on the DTO. Left as raw reads until Plan 05 extends the composer
    // + section DTOs to cover the full compliance surface.
    ...
    $defaultTools = [
        ...
        'PPE: safety footwear, dust masks (FFP3), safety glasses',
...
{{-- ─── Cover: consumes $dto->cover exclusively (Plan 03) ──────────────── --}}
<div class="cover-company-name">{{ strtoupper($companyInfo->name ?: $company) }}</div>
...
@php
    // Client contact display — matches the legacy blade's inline
    // "name<br>email" composition (260726-rf2). Both parts pre-escaped.
    $coverClientContact = trim(
        e($cover->clientContactName)
        . ($cover->clientContactEmail !== '' ? '<br>' . e($cover->clientContactEmail) : '')
    );
@endphp
...
<table class="cover-table">
    ...
    <tr>
        ...
        <td class="val">{{ $cover->client ?: '—' }}</td>
    </tr>
    <tr>
        <td class="lbl">SITE:</td>
        <td class="val">{{ $cover->site ?: '—' }}</td>
    </tr>
    <tr>
        <td class="lbl">PROJECT REFERENCE:</td>
        <td class="val">{{ $cover->projectRef ?: '—' }}</td>
    </tr>
    <tr>
        <td class="lbl">ROOMS:</td>
        <td class="val">
            @if(! empty($cover->rooms))
                @foreach($cover->rooms as $r){{ $r }}<br>@endforeach
            @else
                &mdash;
            @endif
        </td>
    ...
    <tr>
        ...
        <td class="val">{{ $cover->date }}</td>
    </tr>
    {{-- Start / End Date + Time — always rendered so form-set values reach
         the cover. "—" placeholder shown when data is missing.

         NOTE (Plan 05 follow-up): CoverSectionDto currently conflates
         planned_start_date + planned_start_time into a single startDate
         string (falls back to the time when the date is empty). The date
         + time split lives on $project['planned_start_time'] here for now
         — Plan 05 should split them cleanly on the DTO (add startTime /
         endTime fields) so the raw $project fallback can go away. --}}
    <tr>
        <td class="lbl">START DATE:</td>
        <td class="val">
            {{ $cover->startDate ?: '—' }}
            @if(! empty($project['planned_start_time'])) &nbsp; {{ $project['planned_start_time'] }}@endif
        </td>
    </tr>
    <tr>
        ...
        <td class="val">
            {{ $cover->endDate ?: '—' }}
            @if(! empty($project['planned_end_time'])) &nbsp; {{ $project['planned_end_time'] }}@endif
        </td>
    </tr>
    @if($cover->workingHours)
    <tr>
        <td class="lbl">WORKING HOURS:</td>
        <td class="val">{{ $cover->workingHours }}</td>
    </tr>
    @endif
</table>
...
<table class="cover-table">
    ...
    <tr>
        ...
        <td class="val">{{ $cover->preparedBy ?: ($companyInfo->name ?: $company) }}</td>
        <td class="lbl">REVISION:</td>
        <td class="val" style="white-space:nowrap;">{{ $cover->revision }}</td>
    </tr>
    <tr>
        <td class="lbl">TELEPHONE:</td>
        <td class="val">{{ $cover->projectManagerPhone ?: $cover->telephone }}</td>
        <td class="lbl">STATUS:</td>
        <td class="val" style="white-space:nowrap;">{{ $cover->status }}</td>
    </tr>
    <tr>
        <td class="lbl">CLIENT CONTACT:</td>
        <td class="val" colspan="3">{!! $coverClientContact !== '' ? $coverClientContact : 'TBC at site induction' !!}</td>
    </tr>
</table>

{{-- Cover Table 3: PERSONNEL & VEHICLES — always rendered so form-set
     values reach the cover. "—" placeholder shown when data is missing.

     $cover->vehicles / $cover->additionalEngineers are already normalised
     arrays produced by CoverComposer (splits both CSV + newline). --}}
@php
    $coverVehiclesDisplay = ! empty($cover->vehicles) ? implode(', ', $cover->vehicles) : '—';
    $coverAdditionalEngs  = ! empty($cover->additionalEngineers) ? implode(', ', $cover->additionalEngineers) : '';
@endphp
<table class="cover-table">
    <colgroup>
        <col style="width:26%;">
    ...
    <tr>
        ...
        <td class="val">{{ ($cover->projectManager ?: $cover->preparedBy) ?: '—' }}</td>
    </tr>
    <tr>
        <td class="lbl">LEAD ENGINEER:</td>
        <td class="val">{{ $cover->leadEngineer ?: '—' }}</td>
    </tr>
    <tr>
        <td class="lbl">ENGINEERS:</td>
        <td class="val">{{ $coverAdditionalEngs ?: '—' }}</td>
    </tr>
    <tr>
        <td class="lbl">PROGRAMMER:</td>
        <td class="val">{{ $cover->programmer ?: '—' }}</td>
    </tr>
    <tr>
...
{{-- ════════════════════════════════════════════════════════════════════════
     ...
     ════════════════════════════════════════════════════════════════════════ --}}
{{-- ─── Section 1: Document Control — consumes $dto->docControl ────────── --}}
...
<table class="std-table">
    ...
    <tbody>
        @foreach($docControl as $revRow)
        <tr>
            <td>{{ $revRow['rev'] ?: '&nbsp;' }}</td>
            <td>{{ $revRow['date'] }}</td>
            <td>{{ $revRow['author'] ?: '—' }}</td>
            <td>{{ $revRow['description'] ?: 'Initial Issue' }}</td>
            <td>{{ $revRow['status'] }}</td>
        </tr>
        @endforeach
        {{-- Two blank rows preserved from legacy blade so signed prints have
             physical space for hand-written revisions on future issues. --}}
...
{{-- ════════════════════════════════════════════════════════════════════════
     ...
     ════════════════════════════════════════════════════════════════════════ --}}
{{-- ─── Section 2: Company Information — consumes $dto->companyInfo ────── --}}
...
<table class="std-table">
    <tbody>
        ...
        <tr>
            <td class="lbl">Prepared by:</td>
            <td>{{ $cover->preparedBy ?: '—' }}</td>
            <td class="lbl">Direct:</td>
            <td>{{ $cover->projectManagerPhone ?: $cover->telephone }}</td>
        </tr>
    </tbody>
</table>
...
{{-- 260712-twi Task 4 — structured COSHH inventory table with GHS codes --}}
@php $coshhBaseline = (array) ($data['coshh_baseline'] ?? config('rams_tier1.coshh_products', [])); @endphp
@if(! empty($coshhBaseline) && config('rams_tier1.enabled', true))
...
<p class="body-para" style="font-size:8pt; color:#555;">
    GHS/CLP hazard codes: H2xx = physical, H3xx = health, H4xx = environmental.
    Refer to the manufacturer Safety Data Sheet (SDS) for each product on site.
    SDS binder kept in Vehicle 1 tool cabinet and available at request from Site Supervisor.
</p>
@else
<ul class="blist">
    ...
    <li><strong>Dust generated by drilling / cutting</strong> — use FFP3 dust masks when drilling into plasterboard, masonry or MDF — all operatives face-fit tested. Use dust extraction where practicable.</li>
...
{{-- ─── Section 8 body: consumes $dto->companyInfo + $cover ────────────── --}}
<p class="body-para">
    This Risk Assessment and Method Statement has been prepared by {{ $companyInfo->name }} for the project detailed herein.
