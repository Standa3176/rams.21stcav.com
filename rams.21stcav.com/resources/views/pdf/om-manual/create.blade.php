...
@section('content')
    ...
    <div class="alert alert-info" style="margin-bottom:1.5rem;">
        ...
        <strong>Step 1</strong> — Use the project’s reviewed quote data to extract equipment (this is the standard workflow).
        ...
        @if (! $selectedProject)
            <div style="margin-top:.35rem; color:#555; font-size:.85rem;">
                No project selected — you can upload a QuoteWerks PDF below instead.
            </div>
        @endif
    </div>

    @if ($selectedProject && $latestPackage)
        @if ($latestPackage->status === \App\Models\ProjectPackage::STATUS_REVIEWED)
            <div class="card" style="margin-bottom:1.25rem;">
                <div class="section-heading">Use Project Quote Data</div>
                <p style="color:#555; font-size:.875rem; margin-bottom:.75rem;">
                    This project already has reviewed quote data. Use it to create the O&amp;M manual without re-uploading a PDF.
                </p>
                <form method="POST" action="{{ route('om-manuals.from-project', $selectedProject) }}">
                    @csrf
                    <button type="submit" class="btn btn-teal">Use Project Quote Data →</button>
                </form>
            </div>
        @else
            <div class="alert alert-warning" style="margin-bottom:1.25rem;">
                The latest quote for this project hasn’t been reviewed yet.
                Please review it first before creating an O&amp;M manual.
                <a href="{{ route('quote-import.review', $latestPackage) }}" style="color:var(--teal);">Review Quote</a>
            </div>
        @endif
    @endif
    ...
    @if (! $selectedProject)
    <div class="card">
        <div class="section-heading">Upload QuoteWerks Quote PDF (optional)</div>
    ...
    @else
        @if (! $latestPackage)
            <div class="alert alert-warning">
                This project doesn’t have any quote data yet.
                <a href="{{ route('quote-import.create', ['project_id' => $selectedProject->id]) }}"
                   style="color:var(--teal);">Upload a quote</a> to continue.
            </div>
        @endif
    @endif
