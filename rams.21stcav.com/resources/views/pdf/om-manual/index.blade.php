...
@section('content')
    ...
    <div class="page-header">
        <h1 class="page-title">{{ $showDeleted ? 'O&M Manuals — Deleted' : 'O&M Manuals' }}</h1>
        <div style="display:flex;gap:.5rem;align-items:center;">
            @if ($isAdmin)
                @if ($showDeleted)
                    <a href="{{ route('om-manuals.index') }}" class="btn btn-outline btn-sm">← Live Records</a>
                @else
                    <a href="{{ route('om-manuals.index', ['show_deleted' => 1]) }}" class="btn btn-outline btn-sm">🗑 View Deleted</a>
                @endif
            @endif
            @if (! $showDeleted)
    ...
    <div class="card" style="padding:0;overflow:hidden;">
        @if ($showDeleted)
            ...
            @if ($manuals->isEmpty())
                <div class="empty-state"><h3>No deleted O&amp;M Manuals</h3></div>
            @else
                    ...
                    <tbody>
                        ...
                        <tr style="opacity:.6;background:#fff8f8;">
                            <td>
                                ...
                                @if ($manual->project_ref)<br><small style="color:#666;">{{ $manual->project_ref }}</small>@endif
                            </td>
                            <td>{{ $manual->client_name ?? '—' }}</td>
                            @if ($isAdmin)
                                <td>{{ $manual->user?->name ?? '—' }}<br><small style="color:#999;font-size:.75rem;">{{ $manual->user?->email ?? '' }}</small></td>
                            @endif
                            <td style="font-size:.8rem;color:#9CA3AF;white-space:nowrap;">{{ $manual->deleted_at->format('d M Y H:i') }}</td>
                            <td>
                                <div class="actions">
                                    <form method="POST" action="{{ route('om-manuals.restore', $manual->id) }}" style="margin:0;">
                                        @csrf
                ...
                @if ($manuals->hasPages())
                    <div class="pagination-wrap" style="padding:1rem;">{{ $manuals->links() }}</div>
                @endif
            @endif

        @elseif ($manuals->isEmpty())
