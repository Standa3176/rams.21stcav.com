...
<title>Cable Schedule Ready — {{ $schedule->project_name }}</title>
</head>
    ...
    <tr>
        <td style="padding:32px;">
            ...
            <p style="margin:0 0 16px;line-height:1.6;">
                Your Cable Schedule for
                <strong>{{ $schedule->project_name }}</strong>@if($schedule->project_ref) ({{ $schedule->project_ref }})@endif
                has been generated and is ready.
            </p>

            <p style="margin:0 0 16px;">
                Generated: {{ optional($schedule->updated_at)->format('j M Y H:i') }} (UK time)
            </p>

            @if($schedule->source_filename)
                <p style="margin:0 0 16px;">The Cable Schedule is attached to this email.</p>
            @else
                <p style="margin:0 0 16px;">Download from the dashboard:</p>
            @endif

            <p style="margin:0 0 16px;">
                <a href="{{ route('cable-schedules.edit', $schedule) }}" style="color:#007B8A;">View in RAMS Platform</a>
            </p>
