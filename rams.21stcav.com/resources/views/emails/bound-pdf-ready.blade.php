...
<title>Project Drawings Ready — {{ $project->name }}</title>
</head>
    ...
    <tr>
        <td style="padding:32px;">
            ...
            <p style="margin:0 0 16px;line-height:1.6;">
                The bound project drawings PDF for
                <strong>{{ $project->name }}</strong>@if($project->ref) ({{ $project->ref }})@endif
                has been generated and is ready.
            </p>

            <p style="margin:0 0 16px;line-height:1.6;">
                The PDF combines a cover sheet, a drawing register, and every
                current drawing in the project (system schematics + rack elevations)
                in deterministic order — ready for handover or printing.
            </p>

            <p style="margin:0 0 16px;">
                Generated: {{ now()->format('j M Y H:i') }} (UK time)
            </p>

            <p style="margin:0 0 16px;">The bound PDF is attached to this email.</p>

            <p style="margin:0 0 16px;">
                <a href="{{ route('projects.drawings.index', $project->id) }}" style="color:#007B8A;">View in RAMS Platform</a>
            </p>
