...
<title>Worksheet Ready — {{ $worksheet->project_name }}</title>
</head>
    ...
    <tr>
        <td style="padding:32px;">
            ...
            <p style="margin:0 0 16px;line-height:1.6;">
                Your Worksheet for
                <strong>{{ $worksheet->project_name }}</strong>@if($worksheet->project_ref) ({{ $worksheet->project_ref }})@endif
                has been generated and is ready.
            </p>

            <p style="margin:0 0 16px;">
                Generated: {{ optional($worksheet->updated_at)->format('j M Y H:i') }} (UK time)
            </p>

            @if($worksheet->filename)
                <p style="margin:0 0 16px;">The Worksheet is attached to this email.</p>
            @else
                <p style="margin:0 0 16px;">Download from the dashboard:</p>
            @endif

            <p style="margin:0 0 16px;">
                <a href="{{ route('worksheets.show', $worksheet) }}" style="color:#007B8A;">View in RAMS Platform</a>
            </p>
