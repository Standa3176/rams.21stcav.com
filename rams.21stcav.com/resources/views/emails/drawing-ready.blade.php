...
<title>{{ $drawing->kindLabel() }} Ready — {{ $drawing->project->name }}</title>
</head>
    ...
    <tr>
        <td style="padding:32px;">
            ...
            <p style="margin:0 0 16px;line-height:1.6;">
                The {{ $drawing->kindLabel() }} for
                <strong>{{ $drawing->project->name }}</strong>@if($drawing->project->ref) ({{ $drawing->project->ref }})@endif
                (revision {{ $drawing->revisionLabel() }}) has been generated and is ready.
            </p>

            <p style="margin:0 0 16px;">
                Generated: {{ optional($drawing->updated_at)->format('j M Y H:i') }} (UK time)
            </p>

            @if($drawing->filename)
                <p style="margin:0 0 16px;">The drawing file is attached to this email.</p>
            @else
                <p style="margin:0 0 16px;">Download from the project drawings page on the platform.</p>
            @endif

            <p style="margin:0 0 16px;">
                <a href="{{ route('projects.drawings.index', $drawing->project_id) }}" style="color:#007B8A;">View in RAMS Platform</a>
            </p>
