...
<title>RAMS Ready — {{ $rams->project_name }}</title>
</head>
    ...
    <tr>
        <td style="padding:32px;">
            ...
            <p style="margin:0 0 16px;line-height:1.6;">
                Your RAMS document for
                <strong>{{ $rams->project_name }}</strong>@if($rams->project_ref) ({{ $rams->project_ref }})@endif
                has been generated and is ready.
            </p>

            <p style="margin:0 0 16px;">
                Generated: {{ optional($rams->updated_at)->format('j M Y H:i') }} (UK time)
            </p>

            @if($rams->filename)
                <p style="margin:0 0 16px;">The RAMS document is attached to this email.</p>
            @else
                <p style="margin:0 0 16px;">Download from the dashboard:</p>
            @endif

            <p style="margin:0 0 16px;">
                <a href="{{ route('rams.review', $rams) }}" style="color:#007B8A;">View in RAMS Platform</a>
            </p>
